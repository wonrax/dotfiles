...
export interface CacheConfig {
   ...
   * image. Without it each container re-downloads its own /nix — tens of GB
   * per task. Defaults to whether the image is a nix one at all, read off its
   * PATH: the bundled image and anything built on it say yes, a debian image
   * has no store to share and gets no volume.
   */
  nix?: boolean;
...
/**
 * Which version control cuts a repo-backed task's workspace. Both give the
 * agent a checkout of its own beside the operator's, sharing the repository's
 * history: a jj workspace, or a git worktree on a branch made for the task.
 */
export type Vcs = "jj" | "git";

/** a host directory or file made visible inside the task container */
export interface Mount {
  /** absolute path on the host, or one starting `~/`; it has to exist */
  host: string;
  /** absolute path inside the container; `/ws` and `/repo` are the engine's */
  at: string;
  /**
   * Read-only unless this is set. What gets mounted is the operator's, and an
   * agent with a shell treats anything writable as somewhere to put things.
   */
  rw?: boolean;
}

/**
 * An MCP server this task's agents get beside agentflow's own. Either a
 * command the container can run, or a URL it can reach — exactly one.
 */
export interface McpServer {
  command?: string;
  args?: string[];
  env?: Record<string, string>;
  /** a streamable-HTTP server, as the container sees it */
  url?: string;
}
...
export interface SpawnRequest {
  /**
   * Absolute path to a jj or git repo on the host; omit for throwaway scratch
   * tasks. The task works in a checkout of its own cut from it — see `vcs` —
   * and never in the operator's.
   */
  repo?: string;
  /**
   * Which version control cuts the workspace. Detected when omitted: a repo
   * with a `.jj` in it is jj, colocated ones included, since that is how its
   * owner works in it; anything else with a `.git` is git. Name it to get a
   * git worktree out of a colocated repo anyway — what jj cannot carry, such
   * as submodules and LFS, is the usual reason.
   */
  vcs?: Vcs;
  /**
   * The revision the workspace is cut from: a jj revset, or anything git
   * resolves to a commit (`origin/dev`, a tag, a sha). Omitted, it is where
   * the operator's checkout stands — jj's working-copy parents, git's HEAD —
   * which is right for "continue from here" and wrong for a repo whose work
   * always starts from an integration branch the operator is not sitting on.
   *
   * Resolved against the repo as it is at spawn. Nothing is fetched first, so
   * a remote-tracking ref is as fresh as the operator's last fetch; the spawn
   * echoes the commit and its age, which is where a stale one shows.
   */
  base?: string;
  ...
  cache?: CacheConfig;
  /**
   * Docker image the task container is built from (default `agentflow:latest`).
   * For a project whose toolchain is an image rather than something an agent
   * can install mid-turn. The engine drives whatever is in there as root and
   * expects to find: `sh`; `jj` or `git` to match the repo, plus `git-lfs` if
   * the repo uses it; `node` and the agent CLIs the graph names (`claude`,
   * `codex`) when it has agent nodes; `gh` when the task asked for a token.
   * What is missing is reported before the first node rather than by it.
   */
  image?: string;
  /**
   * Host paths the container can see, read-only by default: a skills
   * directory, a test corpus, a sibling checkout the build reads. Not for the
   * repo the task works on — that is `repo`, which gets a checkout of its own
   * instead of the operator's.
   */
  mounts?: Mount[];
  /**
   * MCP servers for this task's agents, by name, beside agentflow's own `af`.
   * They are the only ones: agents run with exactly the servers listed here,
   * never the host's and never a `.mcp.json` found in the checkout, so a
   * server the work depends on has to be named. A `command` runs inside the
   * container, so it has to exist in the image or on a mount.
   */
...
export interface TaskRecord {
  ...
  container?: string;
  /**
   * What cut the workspace, when repo-backed. Absent on records written before
   * git was an option, all of which are jj.
   */
  vcs?: Vcs;
  /**
   * The workspace's name in the operator's repo, when repo-backed: the jj
   * workspace, or the git branch the worktree is on.
   */
  workspace?: string;
  /**
   * The revision this task's workspace was cut from, as one readable line —
   * `request.base` resolved, or where the operator's checkout stood at spawn.
   * Recorded and echoed because the base is invisible until something built on
   * it fails: a task cut from a broken working copy looks perfectly healthy
   * right up to the first gate.
   */
  base?: string;
  /**
   * The same revision as a full commit id, when it is a single commit. Nodes
   * get it as `$AF_BASE`, which is what `git diff` needs to show a task's
   * change and what a shell gate needs to list the files it touched.
   */
  baseCommit?: string;
  /**

...
import type { Base } from "./vcs.ts";
import { detectVcs, repoProblem, resolveBase } from "./vcs.ts";
...
/** a docker image reference: registry, path, tag or digest — and nothing that reads as a flag */
const IMAGE_REF = /^\w[\w.\-/:@]{0,254}$/;

/**
 * What a request says about its repo and its container, checked before the
 * record exists for the same reason the graph is. Every one of these is
 * otherwise a task that sits on the board for the minute it takes to build a
 * container and then fails inside it, in docker's words or git's rather than
 * in terms of the field that was wrong.
 *
 * Returns what the repo resolved to, since working that out is most of the
 * checking: which vcs cuts the workspace, and the revision it is cut from.
 */
const checkEnvironment = (request: SpawnRequest): { vcs?: Vcs; base: Base } => {
  if (request.vcs !== undefined && request.vcs !== "jj" && request.vcs !== "git") {
    throw new TaskError(`vcs is "jj" or "git", got ${JSON.stringify(request.vcs)}`);
  }
  if (request.base !== undefined) {
    // a leading dash is an option to the command this ends up an argument of
    if (typeof request.base !== "string" || !request.base.trim() || request.base.startsWith("-")) {
      throw new TaskError(`base must name a revision, got ${JSON.stringify(request.base)}`);
    }
  }
  if (!request.repo && (request.vcs !== undefined || request.base !== undefined)) {
    throw new TaskError(
      "vcs and base describe the workspace cut from `repo`, and this task has no repo",
    );
  }
  if (
    request.image !== undefined &&
    (typeof request.image !== "string" || !IMAGE_REF.test(request.image))
  ) {
    throw new TaskError(
      `image must be a docker image reference, got ${JSON.stringify(request.image)}`,
    );
  }
  if (request.mounts !== undefined) {
    if (!Array.isArray(request.mounts)) {
      throw new TaskError("mounts is a list of { host, at, rw? }");
    }
  ...
  if (request.mcp !== undefined) {
    if (!request.mcp || typeof request.mcp !== "object" || Array.isArray(request.mcp)) {
      throw new TaskError("mcp maps server names to { command, args?, env? } or { url }");
    }
    for (const [name, server] of Object.entries(request.mcp)) {
      const problem = mcpProblem(name, server);
      if (problem) throw new TaskError(problem);
    }
  }

  if (!request.repo) return { base: {} };
  if (typeof request.repo !== "string") throw new TaskError("repo must be a path");
  const vcs = request.vcs ?? detectVcs(request.repo);
  if (!vcs) {
    throw new TaskError(
      `${request.repo} is neither a jj nor a git repository — there is no .jj or .git in it. ` +
        `For a task that fetches its own code, omit repo and clone in setup.`,
    );
  }
  const problem = repoProblem(request.repo, vcs);
  if (problem) throw new TaskError(problem);
  try {
    return { vcs, base: resolveBase(request.repo, vcs, request.base) };
  } catch (e) {
    throw new TaskError(e instanceof Error ? e.message : String(e));
  }
};
...
export const spawn = (input: SpawnRequest & { id?: string }): SpawnResult => {
  ...
  const environment = checkEnvironment(request);
  ...
  task.vcs = environment.vcs;
  task.base = environment.base.line;
  task.baseCommit = environment.base.commit;
  store.insert(task);
...
const spawnFromNode = (parent: TaskRecord, line: Record<string, unknown>): string => {
  ...
  const { task: child, joined } = spawn({
    ...
    // The container a child runs in is the tree's, for the same reason its
    // model is: host paths and an image are not a container's to choose, and a
    // child that cannot see what its parent was given — the toolchain, the
    // corpus, the spec server the work is judged against — is a different job
    // from the one it was handed. How the repo is cut goes with the repo.
    image: parent.request.image,
    mounts: parent.request.mounts,
    mcp: parent.request.mcp,
    ...(repo ? { vcs: parent.request.vcs, base: parent.request.base } : {}),
...
/**
 * What a workspace teardown left in the operator's repo on purpose, as the tail
 * of a sentence. Only git has anything to say: a branch with commits on it and
 * a snapshot of uncommitted work are the task's output, and the operator should
 * hear where it went from the same message that says the checkout is gone.
 */
const keptNote = (task: TaskRecord, kept: string[]): string =>
  kept.length ? `; kept in ${task.request.repo}: ${kept.join(", ")}` : "";

/**
 * Granular cleanup, keeping the task record and history. container: docker rm
 * (disables revive). workspace: take it out of the repo — jj workspace forget,
 * or the git worktree, keeping whatever holds work — and delete its directory.
...
export const cleanup = async (
  ...
  const kept = opts.workspace ? await Effect.runPromise(teardownWorkspace(task)) : [];
  store.record(id, {
    kind: "control",
    message: `cleaned up ${
...
/**
 * stop + tear down container, workspace, and workspace dir, then forget the
 * task. Returns what was left in the repo on purpose, because the record that
 * would otherwise carry that message is the thing being deleted.
 */
export const remove = async (id: string): Promise<string[]> => {
  const task = need(id);
  await stop(id).catch(() => {});
  const kept = await Effect.runPromise(teardownTask(task));
  await store.drop(id);
  return kept;
};
...
const reap = async (task: TaskRecord) => {
  ...
  const kept = await Effect.runPromise(teardownWorkspace(task)).catch(() => [] as string[]);
  store.record(task.id, {
    kind: "control",
    message: `container and workspace released (retain.container: "${policy}"); the record and ` +
      `its event log stay${keptNote(task, kept)}`,
  });
  store.changed(task.id, { immediate: true });
};

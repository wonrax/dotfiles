  ...
  schema: {
    ...
    properties: {
      ...
      url: {
        type: "string",
        description:
          "the pull request's full URL, restated every turn. Everything downstream identifies the PR by this and has no other way to find it.",
      },
      summary: {
        type: "string",
        minLength: 1,
 ...
 *
 * Nor do they say which version control the workspace is under. What the
 * working directory is, how its diff is read and how work gets recorded are
 * `workspaceNote` in vcs.ts, appended to every agent turn — so "the diff" and
 * "one commit" below are right for a jj workspace and a git worktree alike,
 * and a graph written outside this file is told the same thing these are.
 */

const IMPLEMENT_PROMPT =
  `You are a coding agent in an isolated container, working in a checkout of the
project that was cut for this task.

Task:
{{task}}
{{rubric}}
The operator reviews your work as this workspace's diff, so keep it there as
one commit. Write its message the way the repository's own log reads — it is
the owner's history, so no AI attribution or Co-Authored-By trailers.`;

const REVIEW_PROMPT =
  `You are a strict, adversarial code reviewer. Another agent changed this workspace for the task below, and you are reviewing it blind: you get the task and the diff, never anything the implementer said about its own work. The code has to speak for itself.
...
const REVIEW_FOLLOWUP =
  `The implementer revised the change in response to your last review. Re-read the diff as it stands now and judge it to the same standard: whether what you marked blocking is genuinely resolved, and whether the revision brought anything new with it.
{{#verify.checks}}
...
// left to say.
const POLISH_PROMPT =
  `The review is over and your change is approved — this pass exists only to take the easy wins, so make the ones you agree with and leave the rest, including anything that would change behavior or grow the diff. Changing nothing is a fine outcome. Bring the commit message up to date if you touch anything.
...
const IMPLEMENT_FOLLOWUP =
  `New information since your last turn, below — address it and bring the commit message up to date. Operator notes take priority and permanently extend the task.
{{#review.blocking}}
...
Inspect the diff and run the checks that actually cover what changed. A
failure counts against this change only if this change caused it — a repo that
was already red is not this task's problem, though the next agent still needs
to hear it is red so it doesn't go chasing ghosts.`;

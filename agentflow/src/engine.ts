...
/** the image a task's container is built from: its own, or the bundled one */
const imageOf = (task: TaskRecord) => task.request.image ?? IMAGE;
 ...
 * profile symlinks pointing at store paths that no longer exist. Keyed per
 * image for the same reason now that a task can name its own: two images are
 * two stores.
 *
 * `nix` is whether the image has a store to share, read off the PATH in its
 * config. An image built on nix reaches into /nix from there and one built on
 * debian does not, and the answer survives `FROM agentflow:latest` without
 * anyone having to remember a label. A volume mounted over a /nix that was
 * never there is an empty directory and one more volume to clean up; a nix
 * image without the volume is tens of GB per task, so this is the default that
 * `cache.nix` overrides rather than a thing the operator has to get right.
 */
const nixVolume = (image: string) =>
  Effect.gen(function* () {
    const r = yield* exec([
      "docker",
      "image",
    ...
    const [id, ...env] = r.output.trim().split(" ");
    const digest = id.replace(/^sha256:/, "").slice(0, 12);
    if (!digest) return undefined;
    let nix = false;
    try {
      const vars = JSON.parse(env.join(" ")) as string[] | null;
      nix = (vars ?? []).some((v) => v.startsWith("PATH=") && v.includes("/nix/"));
    } catch {
      // an image with no env block is not a nix one
    }
    return { name: `agentflow-nix-${digest}`, nix };
  });
...
const seedVolume = (vol: string, image: string) =>
  Effect.promise(() => {
 ...
 *
 * A jj workspace shares the repo store it was added from and a git worktree
 * shares its config file, so either way a remote edited from inside the
 * container is the operator's remote — and on this host that config is SSH and
 * is the only credential their own fetch has. The `url.insteadOf` written at
 * container setup removes the *reason* to edit one; this removes an edit's
 * ability to survive, which is the half that does not depend on an agent
 * choosing well.
 *
 ...
 *
 * Runs host-side on purpose — the stored URL is what is read here, where
 * nothing rewrites it; inside a container the same listing reports the
 * rewritten one.
 */
const guardRepoRemotes = (task: TaskRecord) =>
  Effect.gen(function* () {
    ...
    const now = yield* Effect.promise(() => listRemotes(repo, vcs));
    if (!saved || now === undefined || now.trim() === saved.trim()) return [];

    // One "<name> <url>" per line, under either vcs. A line without a space is
    // not that, and guessing at one would restore a remote to nonsense —
    ...
    const have = parse(now);
    const undone: string[] = [];
    for (const [name, url] of want) {
      if (have.get(name) === url) continue;
      yield* Effect.promise(() =>
        setRemote(repo, vcs, have.has(name) ? "set-url" : "add", name, url)
      );
      undone.push(`${name} back to ${url}`);
    }
    for (const name of have.keys()) {
      if (want.has(name)) continue;
      yield* Effect.promise(() => setRemote(repo, vcs, "remove", name));
...
/**
 * `~/x` as the daemon's home has it. Request files are written by hand and
 * carried between machines, and an absolute path into one person's home is
 * the part of them that does not travel.
 */
const hostPath = (p: string) =>
  p === "~" || p.startsWith("~/") ? `${Deno.env.get("HOME")}${p.slice(1)}` : p;

/** where the engine keeps its own things inside a container; a mount over either hides them */
const ENGINE_PATHS = ["/ws", "/repo"];

/** why a mount cannot be made, in words for the spawn that asked for it; nothing when it can */
export const mountProblem = (m: Mount): string | undefined => {
  if (!m || typeof m !== "object" || typeof m.host !== "string" || typeof m.at !== "string") {
    return `a mount is { host, at, rw? } with both paths as strings, got ${JSON.stringify(m)}`;
  }
  const host = hostPath(m.host);
  if (!host.startsWith("/")) return `mount host "${m.host}" must be absolute, or start with ~/`;
  if (!m.at.startsWith("/") || m.at === "/" || m.at.split("/").includes("..")) {
    return `mount target "${m.at}" must be an absolute path inside the container, below /`;
  }
  // `-v` splits on colons, so one in a path is silently a different mount
  if (host.includes(":") || m.at.includes(":")) {
    return `mount ${m.host} → ${m.at}: docker cannot take a colon in either path`;
  }
  const at = m.at.replace(/\/+$/, "");
  if (ENGINE_PATHS.some((p) => at === p || at.startsWith(`${p}/`))) {
    return `mount target "${m.at}" is under ${ENGINE_PATHS.join(" or ")}, which hold the task's ` +
      `own workspace and the repo it was cut from`;
  }
  if (m.rw !== undefined && typeof m.rw !== "boolean") return `mount ${m.at}: rw is a boolean`;
  try {
    Deno.statSync(host);
  } catch {
    return `mount host "${m.host}" does not exist on this machine`;
  }
  return undefined;
};

/**
 * What a task needs to find in its image, given what it is going to do there.
 * Only asked of an image the operator named: the bundled one is built with all
 * of it, and a container missing a tool otherwise says so at the first node to
 * reach for it — `sh: exec: codex: not found`, forty minutes and one
 * implementer turn into the graph.
 */
const requiredTools = (task: TaskRecord, nodes: WorkflowNode[]): string[] => {
  const vcs = vcsOf(task);
  const agents = nodes.flatMap((n) => isAgentExec(n.exec) ? [n.exec as string] : []);
  return [
    ...new Set([
      ...(vcs ? [vcs] : []),
      // the `af` MCP server every agent turn is started with
      ...(agents.length ? ["node"] : []),
      ...agents,
      ...(task.request.gh ? ["gh"] : []),
    ]),
...
/**
 * Created from inside the container so the workspace's pointer file holds
 * container paths. With no base asked for, jj gives the new working copy the
 * same parents as the operator's — including a merge, which is why the default
 * is "no `-r`" rather than the revision `resolveBase` printed.
 */
const addJjWorkspace = (task: TaskRecord, container: string, wc: string) =>
  Effect.gen(function* () {
    const wsName = `af-${task.id}`;
    const from = task.request.base ? ["-r", task.baseCommit ?? task.request.base] : [];
    // deno-fmt-ignore
    yield* execOk([
      "docker", "exec", container,
      "jj", "-R", "/repo", "workspace", "add", "--name", wsName, ...from, wc,
    ]);
...
/**
 * A worktree on a branch of its own, from inside the container for the same
 * reason the jj workspace is: both halves of the link between a worktree and
 * its repository are absolute paths, and the ones that resolve in here are the
 * ones the agent's git will follow. The host therefore never runs git *in* the
 * worktree; it reads the branch and the snapshot ref from the repository.
 *
 * Recorded on the task before the attempt, so a worktree that was half made —
 * a checkout that died on an LFS object, say — is still one teardown knows to
 * look for.
 */
const addGitWorktree = (task: TaskRecord, container: string, repo: string, wc: string) =>
  Effect.gen(function* () {
    const branch = yield* Effect.promise(() => freeBranch(repo, `af-${task.id}`));
    task.workspace = branch;
    task.cwd = wc;
    const fail = (what: string, r: { output: string }, hint = "") =>
      new WorkflowError({
        message: `${what}${hint ? ` ${hint}` : ""}\n${r.output.trim().slice(-1500)}`,
      });

    /**
     * Two ways to find out, one answer. A repo that git-lfs has been used in
     * carries its post-checkout hook, which fails the `worktree add` by name
     * when the binary is missing; one that only has the attributes checks out
     * without a word. Either way the files are already on disk as pointers.
     */
    const noLfs = new WorkflowError({
      message: `${repo} keeps files in git-lfs and the image (${imageOf(task)}) has no ` +
        `git-lfs, so every one of them was checked out as a pointer of about 130 bytes ` +
        `instead of its content. That fails later and as something else — a database that ` +
        `is "not a database", a fixture that will not parse — so it stops here. Add git-lfs ` +
        `to the image; the bundled one carries it from image/Dockerfile as it is now, so an ` +
        `older build wants rebuilding.`,
    });

    // deno-fmt-ignore
    const added = yield* exec([
      "docker", "exec", container, "sh", "-c", GIT_ADD_WORKTREE,
      "sh", branch, wc, task.baseCommit ?? "HEAD",
    ]);
    if (added.code !== 0) {
      if (/'git-lfs' was not found/.test(added.output)) return yield* noLfs;
      return yield* fail(
        `could not cut a git worktree of ${repo} for this task.`,
        added,
        /smudge filter lfs failed/i.test(added.output)
          ? `The checkout stopped on a git-lfs object that is not in the repository's local ` +
            `store. \`git lfs fetch --all\` in ${repo} puts them there; with gh: true the ` +
            `container can also fetch them itself.`
          : "",
      );
    }

    const lfs = yield* dockerExec(container, wc, ["sh", "-c", GIT_LFS_CHECK]);
    if (lfs.output.includes("LFS_WITHOUT_FILTER")) return yield* noLfs;

    const subs = yield* dockerExec(container, wc, ["sh", "-c", GIT_SUBMODULES]);
    if (subs.code !== 0) {
      return yield* fail(
        `the worktree is cut, but its submodules could not be checked out, and a tree missing ` +
          `them fails its first build on a file that is not there.`,
        subs,
        `A private one needs gh: true for the token. One this work can do without is git's to ` +
          `skip: \`git config submodule.<name>.update none\` in ${repo}.`,
      );
    }
  });

/** Create workspace dir + container (+ a checkout of the repo when there is one). Mutates `task`
 * in place so partially-created resources are still recorded for teardown on failure. */
export const setupTask = (task: TaskRecord) =>
  Effect.gen(function* () {
    ...
    // checked at spawn too, but a record outlives the machine state it was
    // checked against: a directory deleted since is a `docker run` that makes
    // an empty one in its place, root-owned, and mounts that
    for (const m of task.request.mounts ?? []) {
      const problem = mountProblem(m);
      if (problem) return yield* new WorkflowError({ message: problem });
      args.push("-v", `${hostPath(m.host)}:${m.at}${m.rw ? "" : ":ro"}`);
    }

    const cache = task.request.cache;
    if (cache?.nix !== false) {
      const vol = yield* nixVolume(image);
      if (vol && (cache?.nix ?? vol.nix)) {
        yield* seedVolume(vol.name, image);
        args.push("-v", `${vol.name}:/nix`);
      }
    }
    ...
    if (task.request.image) {
      const tools = requiredTools(task, nodes);
      // deno-fmt-ignore
      const probe = yield* exec([
        "docker", "exec", name, "sh", "-c",
        'for t in "$@"; do command -v "$t" >/dev/null 2>&1 || printf "%s " "$t"; done',
        "sh", ...tools,
      ]);
      const missing = probe.output.trim();
      if (probe.code !== 0 || missing) {
        return yield* new WorkflowError({
          message: probe.code !== 0
            ? `image ${image} has no usable \`sh\`, which is how the engine runs everything in ` +
              `a task container: ${probe.output.slice(-300)}`
            : `image ${image} has no ${missing.split(/\s+/).join(", ")} on root's PATH, and ` +
              `this task needs ${tools.join(", ")} — what its repo, its graph and its request ` +
              `add up to. Nothing has run yet. See "Task images" in the README for what an ` +
              `image has to carry.`,
        });
      }
      // what the bundled image bakes in: no AI attribution on commits made here
      if (needsClaude) {
        // deno-fmt-ignore
        yield* exec([
          "docker", "exec", name, "sh", "-c",
          "mkdir -p /root/.claude && [ -e /root/.claude/settings.json ] || " +
          "printf '{\"includeCoAuthoredBy\": false}\\n' > /root/.claude/settings.json",
        ]).pipe(Effect.ignore);
      }
    }

    // commits inside the container default to the host operator's identity; a
    // task with no repo gets the jj one, as every task did before git was an option
    const who = yield* Effect.promise(() => hostIdentity(task.request.repo ?? "", vcs ?? "jj"));
    if (vcs === "git") {
      // not best-effort: without `safe.directory` the very next git command
      // refuses the repo, and that error names ownership rather than this
      // deno-fmt-ignore
      yield* execOk([
        "docker", "exec", name, "sh", "-c", GIT_PREPARE, "sh", who?.name ?? "", who?.email ?? "",
      ]);
    } else if (who) {
      // deno-fmt-ignore
      yield* exec([
    ...
    if (task.request.repo && vcs) {
      ...
      // is holding them to.
      const remotes = yield* Effect.promise(() => listRemotes(repo, vcs));
      if (remotes !== undefined) {
        yield* Effect.promise(() =>
          Deno.writeTextFile(`${ws}/repo-remotes.conf`, remotes).catch(() => {})
        );
      }
      const wc = `/ws/${task.id}/wc`;
      if (vcs === "git") yield* addGitWorktree(task, name, repo, wc);
      else yield* addJjWorkspace(task, name, wc);
    }
...
/**
 * Best-effort: take the workspace out of the operator's repo, host-side, then
 * remove its directory. Returns what was deliberately left behind in the repo,
 * in words for the operator — a git branch with commits on it, a snapshot of
 * work that was never committed. jj has nothing to report: forgetting a
 * workspace leaves its commits where they were.
 */
export const teardownWorkspace = (task: TaskRecord): Effect.Effect<string[]> =>
  Effect.gen(function* () {
    let kept: string[] = [];
    const repo = task.request.repo;
    if (task.workspace && repo) {
      ...
      if (vcsOf(task) === "git") {
        kept = yield* Effect.promise(() =>
          forgetGitWorktree(repo, workspace, task.cwd ?? `/ws/${task.id}/wc`, task.baseCommit)
            .catch(() => [] as string[])
        );
      } else {
        yield* exec(["jj", "-R", repo, "workspace", "forget", workspace]).pipe(Effect.ignore);
      }
...
export const teardownTask = (task: TaskRecord) =>
  Effect.gen(function* () {
    ...
    return yield* teardownWorkspace(task);
  });
...
/**
 * Commit id of the workspace as it stands right now, for rerun
 * --reset-workspace — and, either way, what puts the agent's work where the
 * operator can read it from their own repo. Under jj that is a side effect of
 * asking: any jj command snapshots the working copy into the shared store.
 * Git has no such commit, so this writes one; see `GIT_SNAPSHOT`.
 */
export const snapshotCommit = (task: TaskRecord): Effect.Effect<string | undefined, never> =>
  Effect.gen(function* () {
    ...
    const git = vcsOf(task) === "git";
    if (git && !task.workspace) return undefined;
    // deno-fmt-ignore
    const r = yield* dockerExec(task.container, task.cwd, git
      ? ["sh", "-c", GIT_SNAPSHOT, "sh", snapshotRef(task.workspace!)]
      : ["jj", "log", "--no-graph", "-r", "@", "-T", "commit_id"],
    ).pipe(Effect.catchAll(() => Effect.succeed({ code: -1, output: "" })));
    // the last line: stderr shares the stream, and git is free with warnings
 ...
 * file contents in this workspace only — it does not move bookmarks or
 * branches, touch other workspaces, rewrite history, or undo anything the
 * agent did outside the workspace (pushes, PRs, network calls). Callers must
 * say so out loud.
 */
export const restoreWorkspace = (task: TaskRecord, commit: string) =>
  Effect.gen(function* () {
    ...
    const git = vcsOf(task) === "git";
    const r = yield* dockerExec(
      task.container,
      task.cwd,
      git ? ["sh", "-c", GIT_RESTORE, "sh", commit] : ["jj", "restore", "--from", commit],
    );
    if (r.code !== 0) {
      return yield* new WorkflowError({
        message: `restoring the workspace to ${commit} failed (the snapshot may have been ` +
          `garbage collected): ${r.output.slice(-500)}`,
      });
    }
    return r.output;
  });
 ...
 *
 * The nix half is a claim about the bundled image, so it goes only with that
 * image. What an operator's own image carries is theirs to say, in the task or
 * the rubric; telling an agent in a debian container to reach for `nix shell`
 * is the opposite of the help this was written to be.
 */
const tooling = (task: TaskRecord) =>
  (task.request.image
    ? ""
...
that failed — goes in /ws/${task.id}/artifacts (also $AF_ARTIFACTS). That
...
const claudeCmd = (task: TaskRecord, agent: AgentDefaults, ...args: string[]) => [
  ...
  [
    ...
    SCHEDULING,
    tooling(task),
    ...(workspaceNote(task) ? [workspaceNote(task)!] : []),
  ].join("\n\n"),
 ...
 */
/** an MCP server's name reaches a TOML table header and a tool-name prefix, so it is kept to what both take bare */
const MCP_NAME = /^[A-Za-z][\w-]{0,63}$/;

/** why a requested MCP server cannot be set up, in words for the spawn that asked; nothing when it can */
export const mcpProblem = (name: string, s: McpServer): string | undefined => {
  if (!MCP_NAME.test(name)) {
    return `mcp server name "${name}": a letter, then letters, digits, dash or underscore`;
  }
  if (name === "af") return `mcp server name "af" is agentflow's own`;
  if (!s || typeof s !== "object" || Array.isArray(s)) return `mcp.${name} is not an object`;
  const strings = (v: unknown) => Array.isArray(v) && v.every((x) => typeof x === "string");
  if ((s.command === undefined) === (s.url === undefined)) {
    return `mcp.${name} takes exactly one of command (run in the container) or url (reached from it)`;
  }
  if (s.command !== undefined && (typeof s.command !== "string" || !s.command.trim())) {
    return `mcp.${name}.command must be a non-empty string`;
  }
  if (s.url !== undefined && (typeof s.url !== "string" || !/^https?:\/\//.test(s.url))) {
    return `mcp.${name}.url must be an http(s) URL`;
  }
  if (s.args !== undefined && !strings(s.args)) return `mcp.${name}.args must be strings`;
  if (
    s.env !== undefined &&
    (typeof s.env !== "object" || s.env === null || !strings(Object.values(s.env)))
  ) {
    return `mcp.${name}.env must map names to strings`;
  }
  if (s.url !== undefined && (s.args !== undefined || s.env !== undefined)) {
    return `mcp.${name} is a url, which takes no args or env`;
  }
  return undefined;
};

const claudeMcp = (s: McpServer) =>
  s.url !== undefined
    ? { type: "http", url: s.url }
    : { command: s.command, args: s.args ?? [], env: s.env ?? {} };

// JSON's string escapes are a subset of TOML's basic-string ones, bar DEL,
// which JSON leaves raw and TOML refuses
const DEL = String.fromCharCode(127);
const toml = (s: string) => JSON.stringify(s).replaceAll(DEL, "\\" + "u007F");

const codexMcp = (name: string, s: McpServer): string => {
  if (s.url !== undefined) return `[mcp_servers.${name}]\nurl = ${toml(s.url)}\n`;
  const env = Object.entries(s.env ?? {});
  return `[mcp_servers.${name}]\ncommand = ${toml(s.command!)}\n` +
    (s.args?.length ? `args = [${s.args.map(toml).join(", ")}]\n` : "") +
    (env.length
      ? `\n[mcp_servers.${name}.env]\n${
        env.map(([k, v]) => `${toml(k)} = ${toml(v)}`).join("\n")
      }\n`
      : "");
};
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.tryPromise({
      try: async () => {
        ...
        // The task's own servers beside agentflow's, which is last so that it is
        // the one that survives a name nobody should have been able to pick.
        const extra = Object.entries(task.request.mcp ?? {});
        await Deno.writeTextFile(
          `${ws}/mcp.json`,
          JSON.stringify({
            mcpServers: {
              ...Object.fromEntries(extra.map(([name, s]) => [name, claudeMcp(s)])),
              af: { command: "node", args: [`/ws/${task.id}/af-mcp.mjs`] },
            },
          }),
        );
        ...
        const hosted = task.request.hostAgentConfig !== false
          ? `${await Deno.readTextFile(`${Deno.env.get("HOME")}/.codex/AGENTS.md`)
            .catch(() => "")}\n\n${HOUSE_STYLE}\n`
          : "";
        const workspace = workspaceNote(task);
        await Deno.writeTextFile(
          `${home}/AGENTS.md`,
          `${hosted}\n${SCHEDULING}\n\n${tooling(task)}\n${workspace ? `\n${workspace}\n` : ""}`,
        );
...
export const runWorkflow = (
  ...
  Effect.gen(function* () {
    ...
    while (currentId) {
      ...
      // milliseconds. Only under jj, where that index is a prop the engine put
      // there: a git worktree's index is the agent's own staging area, and
      // what goes into it is the agent's call.
      if (vcsOf(task) === "jj") {
      ...
      const nodeEnv: Record<string, string> = {
        ...
        AF_TASK: task.id,
        // what the workspace was cut from: `git diff $AF_BASE` is the task's
        // change, and a shell gate that wants the files it touched has no
        // other way to ask
        ...(task.baseCommit ? { AF_BASE: task.baseCommit } : {}),
...
wrong. Output the object and nothing else.`;
      ...
      const res = yield* nodeExec.pipe(
        ...
        Effect.onInterrupt(() =>
          Effect.promise(() => killTurn(container, task.id, node.id, visit))
        ),
      ...
      if (undone.length) {
        cb.publish({
          ...
          from: "agentflow",
          to: "*",
          message: `a container edited ${task.request.repo}'s git remotes, which its workspace ` +
      ...
      // so the host sees the agent's files immediately, from the operator's own
      // repo: jj snapshots the working copy on any command, git has to be asked
      if (vcsOf(task) === "git") yield* snapshotCommit(task);
      else if (task.request.repo) {

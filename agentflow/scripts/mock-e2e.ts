...
const { id, record } = await af.run({
  ...
  workflow: {
    ...
    nodes: [
      // fails on first visit, succeeds on second — proves the loop edge works
      {
        ...
        type: "shell",
        label: "Work",
        description: "fails on the first visit and passes on the second",
        run:
          "test -f done.flag || { touch done.flag; echo 'first visit: failing'; exit 1; }; echo 'second visit: ok'",
      },
      {
        id: "final",
        type: "shell",
        label: "Final",
        description: "terminal no-op",
        run: "echo all done",
      },
    ],
    edges: [
  ...
  onEvent: (e) => {
    const ev = e as {
      kind: string;
      run?: { node: string; visit: number; status: string };
      task?: { status: string };
    };
    if (ev.kind === "node") {
      console.log(`node ${ev.run!.node} v${ev.run!.visit}: ${ev.run!.status}`);
    }

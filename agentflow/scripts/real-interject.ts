...
const { id, handle } = await af.spawn({
  repo: Deno.args[0],
  task:
    "In convert.py, add a --precision N flag controlling output decimal places (default 1). Keep existing behavior otherwise.",
  gates:
    "python3 -c \"import subprocess as s; assert s.run(['python3','convert.py','100c'],capture_output=True,text=True).stdout.strip()=='212.0f'; print('gates pass')\"",
...
conn.on(
  "node",
  (r: { node: string; visit: number; status: string }) =>
    console.log(`node ${r.node} v${r.visit}: ${r.status}`),
);

// let claude get ~20s into implementing, then interrupt with new requirements
setTimeout(async () => {
  console.log(">>> interjecting with urgent");
  await handle.interject({
    message:
      "Change of plans from the operator: ALSO add kelvin support (unit 'k', e.g. `convert.py 273.15k` prints `0.0c`, celsius converts to kelvin with `--to k`). Do this in the same change.",
    urgent: true,
  });
}, 20_000);

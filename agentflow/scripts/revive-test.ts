...
const { handle } = await af.spawn({
  ...
  workflow: {
    ...
    nodes: [{
      ...
      type: "shell",
      label: "Work",
      description: "appends each visit to runs.log so revival is visible",
      run: 'echo "feedback was: [{{feedback}}]" >> runs.log; cat runs.log',
    }],
    edges: [],
  },
});
...
const final = await new Promise<{ status: string; runs: { output: string }[] }>((res) => {
  conn2.on(
    "update",
    (t: { status: string; runs: { output: string }[] }) =>

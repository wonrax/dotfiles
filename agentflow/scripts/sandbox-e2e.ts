...
try {
  ...
  // -------------------------------------------------------------------------
  console.log("\na git repo gets a worktree\n");

  /**
   * The git half of a repo-backed task, end to end against a real container:
   * what the worktree is cut from, what is in it, what the operator can see of
   * it from their own repo, and what a teardown leaves behind. The fixture is
   * built to trip each thing that went wrong on paper — a base that is not
   * HEAD, a submodule, `commit.gpgsign` in the repo's config with no key
   * anywhere, an agent that rewrites a remote.
   *
   * The submodule's origin sits inside the repo directory, because that is the
   * one host path the container can see: it is `/repo/.sub-origin` in there,
   * and that is the URL `.gitmodules` carries. Cloning it is the file
   * transport, which git refuses for submodules unless told otherwise, so the
   * task passes `protocol.file.allow` through `GIT_CONFIG_*` — which also
   * shows the engine's own entries are added to the operator's, not over them.
   */
  const grepo = await Deno.makeTempDir({ prefix: "agentflow-git-" });
  const git = async (dir: string, ...args: string[]) => {
    // signing off, as for jj above: the host's own config may sign through an
    // agent that has no session in a test run
    const r = await new Deno.Command("git", {
      // deno-fmt-ignore
      args: [
        "-C", dir, "-c", "commit.gpgsign=false", "-c", "tag.gpgsign=false",
        "-c", "user.name=Fixture", "-c", "user.email=fixture@sandbox.test", ...args,
      ],
      stdout: "piped",
      stderr: "piped",
    }).output().catch(() => undefined);
    return { ok: r?.success ?? false, out: new TextDecoder().decode(r?.stdout).trim() };
  };

  await git(grepo, "init", "-q", "-b", "main");
  await git(grepo, "config", "user.name", "Sandbox Operator");
  await git(grepo, "config", "user.email", "operator@sandbox.test");
  const subOrigin = `${grepo}/.sub-origin`;
  await Deno.mkdir(subOrigin);
  await git(subOrigin, "init", "-q", "-b", "main");
  await Deno.writeTextFile(`${subOrigin}/hello.txt`, "from the submodule\n");
  await git(subOrigin, "add", ".");
  await git(subOrigin, "commit", "-qm", "submodule content");
  const subSha = (await git(subOrigin, "rev-parse", "HEAD")).out;
  await Deno.writeTextFile(`${grepo}/.git/info/exclude`, ".sub-origin/\n", { append: true });

  await Deno.writeTextFile(`${grepo}/README.md`, "hi\n");
  await Deno.writeTextFile(
    `${grepo}/.gitmodules`,
    '[submodule "sub"]\n\tpath = sub\n\turl = /repo/.sub-origin\n',
  );
  await git(grepo, "update-index", "--add", "--cacheinfo", `160000,${subSha},sub`);
  await git(grepo, "add", "README.md", ".gitmodules");
  await git(grepo, "commit", "-qm", "the base commit");
  await git(grepo, "tag", "v1");
  const baseSha = (await git(grepo, "rev-parse", "HEAD")).out;
  await Deno.writeTextFile(`${grepo}/second.txt`, "later\n");
  await git(grepo, "add", "second.txt");
  await git(grepo, "commit", "-qm", "the second commit");
  const headSha = (await git(grepo, "rev-parse", "HEAD")).out;
  const SSH_REMOTE = "git@github.com:acme/widgets.git";
  await git(grepo, "remote", "add", "origin", SSH_REMOTE);
  await git(grepo, "config", "commit.gpgsign", "true");
  // not clean: the gitlink was written straight into the index, so the fixture's
  // own checkout has no `sub` directory and says so. What matters is that a
  // task leaves this exactly as it found it.
  const statusBefore = (await git(grepo, "status", "--porcelain")).out;

  if (!subSha || !baseSha || !headSha || baseSha === headSha) {
    console.log("  skip  no usable git on this host");
  } else {
    const fileTransport = {
      GIT_CONFIG_COUNT: "1",
    ...
    const gitTask = await api("/api/spawn", {
      id: "git-task",
      ...
      workflow: shell(
        "git-worktree",
        `set -eu
[ "$(git rev-parse HEAD)" = "$AF_BASE" ] || { echo "HEAD is not AF_BASE ($AF_BASE)"; exit 1; }
[ "$(git rev-parse --abbrev-ref HEAD)" = af-git-task ] || { echo "on $(git rev-parse --abbrev-ref HEAD)"; exit 1; }
[ -f sub/hello.txt ] || { echo "the submodule was not checked out"; exit 1; }
[ ! -e second.txt ] || { echo "cut from HEAD rather than from the base it asked for"; exit 1; }
echo changed >> README.md
echo new > created.txt
git commit -qam "the agent's commit"
git remote set-url origin https://example.invalid/moved.git
echo "GIT-OK $(git log -1 --format='%an <%ae> %G?') count=$GIT_CONFIG_COUNT"`,
      ),
    });
    check(
      "a repo with no .jj in it is detected as git",
      gitTask.body.record?.vcs === "git",
      JSON.stringify(gitTask.body.record?.vcs ?? gitTask.body.error ?? ""),
    );
    check(
      "the base it asked for is resolved at spawn: the commit, what points at it, its age",
      gitTask.body.record?.baseCommit === baseSha &&
        /the base commit/.test(gitTask.body.record?.base ?? "") &&
        /tag: v1/.test(gitTask.body.record?.base ?? "") &&
        /ago$/.test(gitTask.body.record?.base ?? ""),
      JSON.stringify(gitTask.body.record?.base ?? gitTask.body.error ?? ""),
    );

    const gitDone = await settle("git-task", 180);
    check(
      "the worktree is on its own branch at that base, submodule and all",
      gitDone.status === "succeeded" && /GIT-OK/.test(gitDone.outputs?.go ?? ""),
      `${gitDone.status}: ${(gitDone.outputs?.go ?? gitDone.error ?? "").slice(-300)}`,
    );
    check(
      "commits in there are the operator's and unsigned, whatever the repo's config says",
      /GIT-OK Sandbox Operator <operator@sandbox\.test> N /.test(gitDone.outputs?.go ?? ""),
      (gitDone.outputs?.go ?? "").slice(-200),
    );
    check(
      "the engine's git settings are added to the task's own rather than replacing them",
      /count=5$/.test((gitDone.outputs?.go ?? "").trim()),
      (gitDone.outputs?.go ?? "").slice(-40),
    );
    check(
      "what it committed is on the branch in the operator's repo",
      (await git(grepo, "log", "-1", "--format=%s", "af-git-task")).out === "the agent's commit",
      (await git(grepo, "log", "-1", "--format=%s", "af-git-task")).out,
    );
    check(
      "what it did not commit is in the snapshot ref, untracked files included",
      (await git(grepo, "show", "refs/agentflow/af-git-task/snapshot:created.txt")).out === "new",
      (await git(grepo, "ls-tree", "-r", "--name-only", "refs/agentflow/af-git-task/snapshot")).out,
    );
    check(
      "a snapshot goes through a scratch index: the file it captured is still untracked",
      (await git(grepo, "ls-tree", "-r", "--name-only", "af-git-task")).out
        .split("\n").includes("created.txt") === false,
      (await git(grepo, "ls-tree", "-r", "--name-only", "af-git-task")).out,
    );
    check(
      "the worktree is locked, so a prune on the host cannot take it",
      /\nlocked/.test((await git(grepo, "worktree", "list", "--porcelain")).out),
      (await git(grepo, "worktree", "list", "--porcelain")).out,
    );
    check(
      "the operator's own checkout was never touched",
      (await git(grepo, "status", "--porcelain")).out === statusBefore &&
        (await git(grepo, "rev-parse", "HEAD")).out === headSha,
      (await git(grepo, "status", "--porcelain")).out,
    );
    const gitLog = (await api("/api/task/git-task/log")).body as { message: string }[];
    check(
      "a remote the container rewrote is put back, and the operator is told",
      (await git(grepo, "remote", "get-url", "origin")).out === SSH_REMOTE &&
        gitLog.some((e) => /put back: origin back to git@github\.com/.test(e.message)),
      (await git(grepo, "remote", "get-url", "origin")).out,
    );

    const cleaned = await api("/api/task/git-task/cleanup", { container: true, workspace: true });
    const cleanedLog = (await api("/api/task/git-task/log")).body as { message: string }[];
    const keptLine = cleanedLog.findLast((e) => /^cleaned up/.test(e.message))?.message ?? "";
    check(
      "cleanup takes the worktree out of the repo",
      cleaned.status === 200 &&
        !/\/ws\/git-task\/wc/.test((await git(grepo, "worktree", "list", "--porcelain")).out),
      (await git(grepo, "worktree", "list", "--porcelain")).out,
    );
    check(
      "and keeps what holds work, saying so: the branch with a commit, the uncommitted snapshot",
      /kept in .*branch af-git-task \(1 commit on it\)/.test(keptLine) &&
        /refs\/agentflow\/af-git-task\/snapshot/.test(keptLine) &&
        (await git(grepo, "rev-parse", "-q", "--verify", "refs/heads/af-git-task")).ok &&
        (await git(grepo, "rev-parse", "-q", "--verify", "refs/agentflow/af-git-task/snapshot")).ok,
      keptLine,
    );

    // nothing done, nothing kept: a finished task that made no change must not
    // leave a branch behind, or a hundred of them leave a hundred
    const idle = await api("/api/spawn", {
      id: "git-idle",
      task: "change nothing",
      repo: grepo,
      env: fileTransport,
      workflow: shell("idle", "true"),
    });
    check(
      "with no base asked for, the worktree is cut from HEAD",
      idle.body.record?.baseCommit === headSha,
      JSON.stringify(idle.body.record?.base ?? idle.body.error ?? ""),
    );
    await settle("git-idle", 180);
    check(
      "a second task takes a branch of its own beside the kept one",
      (await git(grepo, "rev-parse", "-q", "--verify", "refs/heads/af-git-idle")).ok,
      (await git(grepo, "branch", "--list")).out,
    );
    const removed = await api("/api/task/git-idle/remove", {});
    check(
      "removing a task that changed nothing leaves nothing: no branch, no snapshot",
      Array.isArray(removed.body.kept) && removed.body.kept.length === 0 &&
        !(await git(grepo, "rev-parse", "-q", "--verify", "refs/heads/af-git-idle")).ok &&
        !(await git(grepo, "rev-parse", "-q", "--verify", "refs/agentflow/af-git-idle/snapshot"))
          .ok,
      JSON.stringify(removed.body),
    );

    /**
     * --reset-workspace under git. The node leaves a file behind and fails; the
     * rerun restores the snapshot taken as that node started, so the second
     * visit must not find the file — and would, without the restore, because
     * nothing else ever removes it. The marker that makes the second visit pass
     * lives in refs, beside the checkout, where a restore has no business.
     */
    await api("/api/spawn", {
      id: "git-reset",
      task: "fail once, leaving litter",
      repo: grepo,
      env: fileTransport,
      workflow: shell(
        "reset",
        `[ ! -e litter.txt ] || { echo "LITTER SURVIVED"; exit 1; }
echo x > litter.txt
[ -e "$AF_REFS/once" ] && { echo "SECOND VISIT CLEAN"; exit 0; }
touch "$AF_REFS/once"; exit 1`,
      ),
    });
    const firstTry = await settle("git-reset", 180);
    const rerun = await api("/api/task/git-reset/rerun", { from: "go", resetWorkspace: true });
    const secondTry = await until(
      "git-reset",
      (t) => ["succeeded", "failed"].includes(t.status) && (t.runs?.length ?? 0) > 1,
      240,
    );
    check(
      "rerun --reset-workspace puts a git worktree's files back, new ones included",
      firstTry.status === "failed" && rerun.status === 200 && secondTry.status === "succeeded" &&
        /SECOND VISIT CLEAN/.test(secondTry.outputs?.go ?? ""),
      `${firstTry.status} → ${rerun.status} ${rerun.body.error ?? ""} → ${secondTry.status}: ` +
        (secondTry.outputs?.go ?? "").slice(-120),
    );
    await api("/api/task/git-reset/remove", {});
    await api("/api/task/git-task/remove", {});

    /**
     * What a git task's agents are told about where they are. It rides on the
     * appended system prompt rather than in any node's template, so the stub
     * over `claude` records its arguments this time, and a shell node reads
     * them back. The same stub trick as the prompt assertions further down.
     */
    await api("/api/spawn", {
      id: "git-note",
      task: "be told where you are",
      repo: grepo,
      env: fileTransport,
      setup: {
        run: [
        ...
        nodes: [
          {
            ...
            session: "none",
            label: "Agent",
            description: "a turn whose arguments are the point",
            run: "anything",
          },
          {
            id: "go",
            exec: "shell",
            label: "Read back",
            description: "prints what the agent was started with",
            run: 'cat "$AF_REFS/claude-args.txt"',
          },
        ],
        edges: [{ from: "agent", to: "go" }],
      },
    });
    const told = (await settle("git-note", 180)).outputs?.go ?? "";
    check(
      "a git task's agents are told it is a worktree, which branch, and what it was cut from",
      /git worktree of the operator's repository/.test(told) && /`af-git-note`/.test(told) &&
        told.includes(headSha.slice(0, 12)) && /git diff \$AF_BASE/.test(told) &&
        !/jujutsu/.test(told),
      told.slice(-500),
    );
    await api("/api/task/git-note/remove", {});
  }

  /**
   * LFS, from the repo's own store: the object is put there by the host's
   * git-lfs and no server exists, so a checkout that has the content got it
   * locally. Which of two right answers comes back depends on the image this
   * host has built — one with git-lfs smudges the file, one without is refused
   * before its first node rather than handed a pointer — so the image is asked
   * first and the matching one asserted. Skipped where the host has no git-lfs
   * to build the fixture with.
   */
  const lrepo = await Deno.makeTempDir({ prefix: "agentflow-lfs-" });
  const LFS_BODY = "not a pointer: ".padEnd(4096, "x");
  const lfs = [
  ...
  await git(lrepo, "init", "-q", "-b", "main");
  await Deno.writeTextFile(
    `${lrepo}/.gitattributes`,
    "*.bin filter=lfs diff=lfs merge=lfs -text\n",
  );
  await Deno.writeTextFile(`${lrepo}/big.bin`, LFS_BODY);
  await git(lrepo, ...lfs, "add", ".gitattributes", "big.bin");
  await git(lrepo, ...lfs, "commit", "-qm", "a file in lfs");
  const pointer = (await git(lrepo, "cat-file", "-p", "HEAD:big.bin")).out;
  if (!/^version https:\/\/git-lfs/.test(pointer)) {
    console.log("  skip  no git-lfs on this host to build the fixture with");
  } else {
    const imageHasLfs = await new Deno.Command("docker", {
      args: ["run", "--rm", "agentflow:latest", "sh", "-c", "command -v git-lfs"],
      stdout: "null",
      stderr: "null",
    }).output().then((r) => r.success, () => false);
    await api("/api/spawn", {
      id: "git-lfs",
      task: "read a file that lives in lfs",
      repo: lrepo,
      workflow: shell("lfs", 'echo "LFS-SIZE $(wc -c < big.bin)"; head -c 14 big.bin'),
    });
    const lfsDone = await settle("git-lfs", 180);
    if (imageHasLfs) {
      check(
        "an lfs file is checked out as its content, from the repo's own store",
        lfsDone.status === "succeeded" && /LFS-SIZE\s*4096/.test(lfsDone.outputs?.go ?? "") &&
          /not a pointer:/.test(lfsDone.outputs?.go ?? ""),
        `${lfsDone.status}: ${(lfsDone.outputs?.go ?? lfsDone.error ?? "").slice(-200)}`,
      );
    } else {
      check(
        "a repo that uses lfs is refused on an image with no git-lfs, not handed pointers",
        lfsDone.status === "failed" && /has no git-lfs/.test(lfsDone.error ?? "") &&
          !(lfsDone.runs?.length),
        `${lfsDone.status}: ${(lfsDone.error ?? "").slice(0, 200)}`,
      );
    }
    await api("/api/task/git-lfs/remove", {});
  }
  await Deno.remove(lrepo, { recursive: true }).catch(() => {});

  // refused at spawn, in terms of the field that was wrong, before any container
  const notRepo = await Deno.makeTempDir({ prefix: "agentflow-norepo-" });
  const refuse = async (name: string, req: Record<string, unknown>, want: RegExp) => {
    const r = await api("/api/spawn", {
      task: "never runs",
      workflow: shell("no", "true"),
      ...req,
    });
    check(
      name,
      r.status === 400 && want.test(r.body.error ?? ""),
      `${r.status} ${r.body.error ?? ""}`,
    );
  };
  await refuse(
    "a repo that is neither jj nor git is refused",
    { repo: notRepo },
    /neither a jj nor a git/,
  );
  await refuse(
    "so is a base that does not resolve",
    { repo: grepo, base: "no-such-rev" },
    /base "no-such-rev" does not resolve/,
  );
  await refuse("and a base with no repo to be a revision of", { base: "main" }, /has no repo/);
  await refuse(
    "a mount whose host path is not there is refused",
    { mounts: [{ host: `${notRepo}/missing`, at: "/mnt/x" }] },
    /does not exist/,
  );
  await refuse(
    "so is one aimed at the engine's own directories",
    { mounts: [{ host: notRepo, at: "/ws/elsewhere" }] },
    /under \/ws or \/repo/,
  );
  await refuse(
    "an MCP server cannot take agentflow's own name",
    { mcp: { af: { command: "true" } } },
    /agentflow's own/,
  );
  await refuse(
    "and has to be a command or a url",
    { mcp: { spec: {} } },
    /exactly one of command/,
  );
  await refuse(
    "an image that reads as a flag is refused",
    { image: "--privileged" },
    /image must be/,
  );

  /**
   * The container knobs, through a real container: a read-only mount, an MCP
   * server beside agentflow's own, and an image named on the request — the
   * bundled one, which is enough to take the path a custom image takes.
   */
  await Deno.writeTextFile(`${notRepo}/hello`, "from the host\n");
  await api("/api/spawn", {
    id: "knobs-task",
    task: "look around",
    image: "agentflow:latest",
    mounts: [{ host: notRepo, at: "/mnt/fixture" }],
    mcp: { spec: { command: "spec-server", args: ["--stdio"], env: { SPEC_BACKEND: "fts5" } } },
    workflow: shell(
      "knobs",
      `cat /mnt/fixture/hello
touch /mnt/fixture/written 2>/dev/null && echo "MOUNT IS WRITABLE"
cat /ws/knobs-task/mcp.json`,
    ),
  });
  const knobs = await settle("knobs-task", 180);
  const knobsOut = knobs.outputs?.go ?? "";
  check(
    "a mount is visible in the container and read-only by default",
    /from the host/.test(knobsOut) && !/MOUNT IS WRITABLE/.test(knobsOut),
    `${knobs.status}: ${knobsOut.slice(0, 200)} ${knobs.error ?? ""}`,
  );
  check(
    "the task's MCP servers are in the agents' config beside agentflow's own",
    /"spec":\{"command":"spec-server","args":\["--stdio"\],"env":\{"SPEC_BACKEND":"fts5"\}\}/.test(
      knobsOut,
    ) &&
      /"af":\{"command":"node"/.test(knobsOut),
    knobsOut.slice(-300),
  );
  await api("/api/task/knobs-task/remove", {});
  await Deno.remove(notRepo, { recursive: true }).catch(() => {});
  await Deno.remove(grepo, { recursive: true }).catch(() => {});

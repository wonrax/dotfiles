 ...
 *   jj and git mechanics, and troubleshooting.
 *
 * ## Writing a graph
 *
...
export interface CacheConfig {
  /**
   * Share one nix store across containers built from the same image. Defaults
   * to whether the image is a nix one, read off its PATH — true for the bundled
   * image and anything built on it, false for a debian one.
   */
  nix?: boolean;
  volumes?: CacheVolume[];
...
export interface Mount {
  ...
  host: string;
  /** absolute path inside the container; not under `/ws` or `/repo` */
  at: string;
  /** read-only unless set */
  rw?: boolean;
}

/** an MCP server for the task's agents: a command run in the container, or a url it can reach */
...
export interface SpawnRequest {
   ...
   * tasks. The agent gets a checkout of its own, cut from the repo and sharing
   * its history — a jj workspace, or a git worktree on a new branch — with no
   * setup on your side. Uncommitted changes in the user's checkout are NOT
   * included, their checkout is never touched, and parallel tasks on one repo
   * are safe. The work shows up live in the user's own repo as `af-<taskId>`:
   * `jj diff -r af-<taskId>@`, or `git log af-<taskId>`. `af help workspace`
   * has the rest: submodules, LFS, what teardown keeps.
   */
  repo?: string;
  /**
   * Which version control cuts the workspace. Detected when omitted — a repo
   * with a `.jj` is jj, colocated ones included; anything else is git. Set
   * "git" on a colocated repo for what jj cannot carry: submodules, LFS.
   */
  vcs?: "jj" | "git";
  /**
   * Revision the workspace is cut from: a jj revset, or anything git resolves
   * to a commit (`origin/dev`, a tag, a sha). Default: where the user's
   * checkout stands — jj's working-copy parents, git's HEAD. Nothing is
   * fetched first. Nodes get the resolved commit as `$AF_BASE`.
   */
  base?: string;
  /**
   * Docker image for the task container (default `agentflow:latest`), for a
   * project whose toolchain is an image. Must work as root and carry what the
   * task uses: jj or git (plus git-lfs), node and the agent CLIs for agent
   * nodes, gh with `gh: true`. Missing tools fail the task before its first
   * node. See `af help workspace`.
   */
  image?: string;
  /** host paths the container can see, read-only by default; not for the repo itself */
  mounts?: Mount[];
  /**
   * MCP servers for the task's agents, by name, beside agentflow's own. These
   * are the only ones they get — the host's are never used and a `.mcp.json`
   * in the checkout is not read.
   */
...
export interface TaskRecord {
  ...
  container?: string;
  /** what cut the workspace, when repo-backed; absent on older records, which are jj */
  vcs?: "jj" | "git";
  /** the workspace's name in the user's repo: the jj workspace, or the git branch */
  workspace?: string;
  cwd?: string;
  /**
   * The revision the workspace was cut from, as one readable line:
   * `request.base` resolved, or where the user's checkout stood at spawn.
   * Worth reading before anything else when a task fails in ways the change
   * cannot explain.
   */
  base?: string;
  /** the same revision as a full commit id, when it is a single commit; `$AF_BASE` in nodes */
  baseCommit?: string;
  /**
...
export interface TaskHandle {
   ...
   * rm (disables revive). workspace: taken out of the repo (jj workspace
   * forget, or the git worktree) and its dir deleted; whatever holds work
   * stays in the repo. Refused (throws) while the task is running.
   */
  cleanup(opts: { container?: boolean; workspace?: boolean }): Promise<TaskRecord>;
  /**
   * stop + full teardown + drop the task from the board. `kept` is what was
   * left in the repo on purpose — under git, a branch with commits on it or a
   * snapshot of uncommitted work; always empty under jj, whose commits simply
   * stay.
   */
  remove(): Promise<{ kept: string[] }>;
...
export interface AgentflowClient {
   ...
   * names the container (`af-<id>`) and the workspace too — the jj workspace
   * or git branch — both of which the operator reads in their own repo weeks
   * later. Omitted = random. Reusing an
...
export function connect(endpoint = DAEMON): AgentflowClient {
  const task = (id: string): TaskHandle => ({
    ...
    remove: () =>
      post(endpoint, `/api/task/${id}/remove`).then((r) => ({
        kept: Array.isArray((r as { kept?: unknown })?.kept) ? (r as { kept: string[] }).kept : [],
      })),
    connect: (opts) =>
      eventStream(

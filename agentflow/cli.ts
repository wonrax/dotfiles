...
const REPEATABLE: Record<string, Set<string>> = {
  spawn: new Set(["child-workflow", "meta", "mount"]),
...
/**
 * A path as the daemon will need it. It may be on the far side of
 * AGENTFLOW_URL and is never in this directory, so "." means nothing to it;
 * `~/` is left alone because the daemon expands that against its own home,
 * which is the one the path has to exist in.
 */
const absolute = (p: string) =>
  p.startsWith("/") || p === "~" || p.startsWith("~/")
    ? p
    : decodeURIComponent(new URL(p, `file://${Deno.cwd()}/`).pathname).replace(/(.)\/$/, "$1");

/** `host:at` or `host:at:rw` into a mount; read-only unless it says otherwise */
const mount = (spec: string) => {
  const [host, at, mode, ...rest] = spec.split(":");
  if (!host || !at || rest.length || (mode !== undefined && mode !== "rw" && mode !== "ro")) {
    throw new UserError(`--mount wants HOST:CONTAINER[:rw], got "${spec}"`);
  }
  return { host: absolute(host), at, ...(mode === "rw" ? { rw: true } : {}) };
};

/**
 * A request file is JSON, or a module whose default export is the request. The
 * module form is for graphs with prompts in them: a prompt is paragraphs of
 * prose, and as a JSON string it is one line of `\n`s nobody can review or
 * diff. Evaluated here, by the caller's own deno, and sent as the plain object
 * it produces — the daemon never runs anything it was handed.
 */
const readRequest = async (file: string): Promise<Record<string, unknown>> => {
  if (!/\.[cm]?[jt]s$/.test(file)) return JSON.parse(await Deno.readTextFile(file));
  const mod = await import(new URL(absolute(file), "file:///").href);
  const req = mod.default;
  if (!req || typeof req !== "object" || Array.isArray(req)) {
    throw new UserError(`${file} has to default-export the request object`);
  }
...
const commands: Record<string, (a: Args) => Promise<void> | void> = {
  ...
  async spawn({ flags }) {
    ...
    const req = file ? await readRequest(file) : {
      id: str(flags.id),
      title: str(flags.title),
      key: str(flags.key),
      task: need(str(flags.task), "--task"),
      repo: str(flags.repo) && absolute(str(flags.repo)!),
      vcs: str(flags.vcs),
      base: str(flags.base),
      image: str(flags.image),
      mounts: list(flags.mount)?.map(mount),
    ...
    if (Object.keys(childWorkflows).length) {
      req.childWorkflows = { ...(req.childWorkflows as object | undefined), ...childWorkflows };
    }
    // the request is whatever the file or the flags said; the daemon is what
    // checks it, so that one validator answers for both
    const { id, record, joined } = await af.spawn(req as unknown as Parameters<typeof af.spawn>[0]);
  ...
  async rm({ _ }) {
    const { kept } = await af.task(need(_[0], "task id")).remove();
    out(`removed ${_[0]}`);
    // a git branch with commits on it, a snapshot of work never committed: the
    // task is gone and what it made is not, and this is the last place to say so
    if (kept.length) out(`  kept in its repo: ${kept.join(", ")}`);
  },
...
CONTROL
  af spawn --task "..." [--repo P] [--title T] [--gates C] [--workflow W]
           ...
           [--vcs jj|git] [--base REV]  how --repo is cut: a jj workspace or a git worktree
                                        (detected; jj wins when colocated), and from which
                                        revision (default: where your checkout stands)
           [--image IMG]                the container's image, for a toolchain that is one
           [--mount HOST:CONTAINER[:rw]]
                                        repeatable: a host path the container can see,
                                        read-only unless :rw. See af help workspace
  ...
  af spawn --file req.json|req.ts       full SpawnRequest, for anything the flags miss; a
                                        module default-exports it, so prompts stay prose
...
TOPICS   af help <topic>
  ...
  workspace  jj or git, the base, what teardown keeps; custom images, mounts, MCP servers
...
does not move bookmarks or branches, un-push one, close a PR, delete a
release, stop a container the agent started, or undo an API call. The world
outside the workspace is not revertible, and this makes no attempt to pretend
otherwise. If the agent pushed a bookmark or a branch, the workspace files go
back but it and the remote do not — leaving the two disagreeing.
...
`,
  ...
  workspace: `WORKSPACES: REPOS, IMAGES, MOUNTS

A task with a repo never works in your checkout. It gets one of its own, cut
from the same repository and sharing its history, and which kind depends on
what the repo is:

  jj    a jj workspace, af-<id>. Chosen when the repo has a .jj in it,
        colocated ones included.
  git   a git worktree on a new branch, af-<id>. Chosen otherwise, or with
        --vcs git (vcs: "git") on a colocated repo — for what jj cannot carry:
        submodules, LFS.

Either way it is created from inside the container, so the paths linking it to
the repository are container paths. Never run jj or git inside
~/.local/state/agentflow/ws/<id>/wc on the host; read the work from your own
repo instead, where it shows up live:

  jj    jj log, jj diff -r af-<id>@
  git   git log af-<id>                  what it committed
        git diff <base> refs/agentflow/af-<id>/snapshot
                                         everything, committed or not, as of
                                         the last node to finish

That snapshot ref is the engine's. Git has no working-copy commit, so after
every node the worktree's state is written as a commit under that one ref:
changes to tracked files, plus new files that are not ignored and are under
10 MiB. The agent's own staging area is not touched. It is also what
af rerun --reset-workspace restores from.

THE BASE. --base REV (base:) is the revision the workspace is cut from: a jj
revset, or anything git resolves to a commit — origin/dev, a tag, a sha. Left
out, it is where your checkout stands: jj's working-copy parents, git's HEAD.
Nothing is fetched first, so origin/dev is as fresh as your last fetch; af spawn
prints the commit and how old it is. Nodes get the commit as $AF_BASE, which is
what makes "git diff $AF_BASE" the task's change.

WHAT GIT MODE SETS UP. Submodules are checked out, recursively, when the
worktree is cut, and a failure there fails the task before its first node. A
private one needs --gh for the token; one the work can do without is git's to
skip: git config submodule.<name>.update none, in the repo. LFS files are
smudged at checkout from the repo's own .git/lfs, so nothing you already have
is downloaded again, and a repo that uses LFS is refused on an image with no
git-lfs — the alternative is a tree of 130-byte pointers that fails later, as
something else. Inside the container your identity is set, and commit signing
and auto-gc are off, all through the environment: the repo's config is shared
with your checkout, along with its branches and remotes, and is left alone.
Remotes a container edits are put back after every node, under either vcs.

TEARDOWN. af cleanup --workspace and af rm take the workspace out of your repo
and keep whatever holds work. Under jj the commits stay where they were. Under
git the branch stays if it has commits on it, and the snapshot ref stays if it
holds work that was never committed; a branch still at its base and a snapshot
equal to the branch are residue and go, so a hundred finished tasks do not
leave a hundred branches. What was kept is in the cleanup event and in what
af rm prints.

IMAGES. --image IMG (image:) replaces agentflow:latest, for a project whose
toolchain is an image rather than something an agent installs mid-turn. The
engine runs everything as root through sh, and needs on root's PATH: jj or git
to match the repo, plus git-lfs if the repo uses it; node and the agent CLIs the
graph names — claude, codex — if it has agent nodes; gh with --gh. What is
missing is reported before the first node. The shared /nix volume follows the
image: on when its PATH reaches into /nix, off otherwise, cache.nix to
override. The note that tells agents about "nix shell" goes only with the
bundled image; what yours carries is yours to say, in the task or the rubric.

MOUNTS AND MCP SERVERS. For what the work needs that is neither the repo nor
the image — a skills directory, a test corpus, the spec server a change is
judged against:

  mounts: [{ host: "~/corp/skills/rust", at: "/root/.claude/skills/rust" }]
  mcp:    { spec: { command: "uv",
                    args: ["--directory", "/opt/spec", "run", "server.py"] } }

Mounts are read-only unless rw: true, and cannot land under /ws or /repo. An
MCP server is a command run inside the container, or a url it can reach. These
are the only servers agents get beside agentflow's own: the host's are never
used and a .mcp.json in the checkout is not read, so a server the work depends
on has to be named. mcp has no flag; it goes in a request file.

Children a task spawns inherit its image, mounts and MCP servers, and how its
repo is cut. None of those is a container's to choose.
`,
...
cache: { nix: false }. An image of your own gets the same when it is a nix one
— its PATH reaches into /nix — and no volume when it is not.

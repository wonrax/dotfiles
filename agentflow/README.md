...
on a checkout of the target repo cut for it — a jujutsu workspace or a git worktree — driven
through a workflow graph (implement, verify,
...
const { id, record } = await af.run({
  repo: "/abs/path/to/repo", // jj or git; omit for a throwaway scratch dir
...
af help workflow|model|prompt|monitor|rerun|cache|artifacts|workspace
```
...
That reset reverts **files in this workspace and nothing else**. It does not move bookmarks or
branches, un-push one, close a PR, or undo an API call — and if the agent pushed one, the files roll
back while it and the remote do not, leaving them disagreeing. The defense against re-running
...
An image of your own (`image:`) gets a volume of its own the same way when it is a nix image — its
PATH reaches into `/nix` — and none when it is not; `cache.nix` overrides either answer.

Setup also runs `git init` and `git add -A` in each jj workspace, which is why that volume no longer
...
block. It sits beside the checkout rather than inside it, so a note one agent leaves for another
...
## How the repo side works

A task with a `repo` never works in the operator's checkout. The container mounts the repo at
`/repo` and an empty task dir at `/ws/<id>`, and the engine cuts a checkout of its own at
`/ws/<id>/wc` from inside the container, so the paths linking it to the repository are container
paths. Never run jj or git inside that directory on the host; it only resolves inside the
container. Which kind of checkout depends on the repo (`vcs:` overrides, `src/vcs.ts` is the code):

- **jj** — a repo with a `.jj`, colocated ones included. `jj workspace add` makes workspace
  `af-<id>`. All commits and the op log live in the shared repo store, so plain `jj log` /
  `jj diff -r af-<id>@` in the repo on the host shows the agent's work live. Teardown runs
  `jj workspace forget`; the commits stay.
- **git** — anything else, or `vcs: "git"` on a colocated repo for what jj cannot carry: submodules
  and LFS. `git worktree add -b af-<id>` makes a worktree on a branch of its own, locked at once —
  its path exists only in the container, which to `git worktree prune` on the host looks exactly
  like a checkout somebody deleted. Submodules are checked out recursively, LFS files are smudged
  from the repo's own `.git/lfs`, and a repo that uses LFS is refused on an image without `git-lfs`
  rather than checked out as pointer files. Git has no working-copy commit, so after every node the
  engine writes the worktree's state as a commit under `refs/agentflow/af-<id>/snapshot`, using a
  scratch index so the agent's staging area is left alone: `git log af-<id>` is what the task
  committed, and `git diff <base> refs/agentflow/af-<id>/snapshot` is everything, committed or not.
  That ref is also what `--reset-workspace` restores from. Teardown removes the worktree and keeps
  whatever holds work: the branch if it has commits on it, the snapshot if it holds work that was
  never committed. A branch still at its base and a snapshot equal to the branch are residue and go.

`base:` names the revision either kind is cut from — a jj revset, or anything git resolves to a
commit, such as `origin/dev`. Left out, it is where the operator's checkout stands. Nothing is
fetched first; the spawn echoes the commit and, for git, its age. Nodes get it as `$AF_BASE`.

Both share more than history with the operator's checkout: a jj workspace shares the repo store and
a git worktree shares `.git/config`, so a remote edited inside a container is the operator's remote.
The engine removes the reasons to touch that config rather than policing it — `url.insteadOf` for
the HTTPS token, and under git an environment that turns commit signing and auto-gc off, since the
signing key is not in the container and a repack of the operator's object store is not the
container's to do — and re-asserts the remotes after every node for the edit that happens anyway.

What an agent is told about all this is one paragraph, `workspaceNote`, appended to every agent
turn: what the directory is, how the task's diff is read, how work gets recorded. The bundled
prompts say "the diff" and "one commit" and leave the rest to it, which is also how a graph written
outside this repo gets told.

## Task images, mounts and MCP servers

`image:` replaces `agentflow:latest` for a project whose toolchain is an image rather than something
an agent can install mid-turn. The engine drives whatever is in there as root, through `sh`, and
needs on root's PATH: `jj` or `git` to match the repo, plus `git-lfs` if the repo uses it; `node`
and the agent CLIs the graph names (`claude`, `codex`) when it has agent nodes; `gh` with
`gh: true`. It checks for those right after the container starts, so a missing one is a task that
fails in seconds with a list, not at the first node to reach for it. The paragraph that tells
agents about `nix shell` goes only with the bundled image — what another image carries is the
operator's to say, in the task or the rubric.

`mounts: [{ host, at, rw? }]` makes host paths visible in the container, read-only unless `rw`, and
never under `/ws` or `/repo`. `mcp: { name: { command, args?, env? } | { url } }` adds MCP servers
beside agentflow's own `af`; they are written into the same strict config, so they are the only
ones agents get — the host's are never used and a `.mcp.json` in the checkout is not read. A child
task inherits its parent's image, mounts and MCP servers along with its model: none of them is a
container's to choose. `af help workspace` is the reference for all of it.
